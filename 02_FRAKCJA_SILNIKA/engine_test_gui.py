...
def main():
    ...
    agent_script_path = os.path.join(main_dir, '03_FRAKCJA_AGENTOW', 'agent007', 'run_agent.py')
    random_script_path = os.path.join(main_dir, '03_FRAKCJA_AGENTOW', 'random_agent.py')

    if not os.path.exists(agent_script_path):
        print(f"BŁĄD: Nie znaleziono skryptu Twojego agenta w: {agent_script_path}")
        return
    if not os.path.exists(random_script_path):
        print(f"BŁĄD: Nie znaleziono skryptu randomowego agenta w: {random_script_path}")
        return
    ...
    game_loop = GameLoop(
        headless=False,
        spawn_points={
            1: TEAM_A_SPAWN_POINTS[:1],
            2: TEAM_B_SPAWN_POINTS[:1]
        }
    )
    # game_loop = GameLoop(
    #     headless=False,
    ...
    try:
        ...
        cmd_agent = [sys.executable, agent_script_path, "--port", "8001", "--name", "Agent007"]
        proc_agent = subprocess.Popen(cmd_agent, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        agent_processes.append(proc_agent)
        print(f"  -> Twój Agent uruchomiony na porcie 8001 (PID: {proc_agent.pid})")

        # Random Agent (Port 8002)
        cmd_random = [sys.executable, random_script_path, "--port", "8002", "--name", "RandomBot"]
        proc_random = subprocess.Popen(cmd_random, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        agent_processes.append(proc_random)
        print(f"  -> Random Agent uruchomiony na porcie 8002 (PID: {proc_random.pid})")
